...
from qmharness_mcp.policy import check_allowlisted, resolve_timeout_s, truncate_data
...
def init(config: McpServerConfig) -> None:
    """Binds the process-wide server config. One `qmharness-mcp` server per process."""
    global _config
    _config = config
...
def _current_config() -> McpServerConfig:
    if _config is None:
        raise RuntimeError("qmharness_mcp.tools.dispatch.init() must be called before dispatch()")
    return _config


def dispatch(tool: str, run: Callable[[int], dict[str, Any]]) -> dict[str, Any]:
    """Shared cross-cutting logic for every `qm.*` tool (M1, M4, M5, M6 of 03-INTERFACES.md §6.2).
