...
from codeintel_mcp.tools import dispatch as dispatch_mod
...
def test_dispatch_returns_an_ok_envelope_on_success() -> None:
    ...
    result = dispatch("code.find_symbol", lambda timeout_s: {"ok": True, "timeout_s": timeout_s})
    assert result == {
        "ok": True,
...
def test_dispatch_rejects_a_tool_outside_the_allowlist() -> None:
    dispatch_mod.init(_config(tools_allowlist=["code.grep"]))
    with pytest.raises(PermissionDeniedError):
        dispatch("code.find_symbol", lambda timeout_s: {})
...
def test_dispatch_maps_an_app_error_to_a_tool_error_the_model_can_read() -> None:
    ...
    def _boom(timeout_s: int) -> dict[str, object]:
        raise DependencyError("no usable index", details={})

    with pytest.raises(ToolError, match="no usable index"):
        dispatch("code.find_symbol", _boom)
...
def test_dispatch_truncates_an_oversized_result() -> None:
    ...
    result = dispatch("code.find_symbol", lambda timeout_s: {"blob": "x" * 1_000})
